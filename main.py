...
def main():
    """
    Main function for the AI Diagnostic System using voice-only input.
    """
    # Initialize Text-to-Speech engine
    engine = pyttsx3.init()

    # Collect patient data
    user_data = collect_patient_data()
    if not user_data:
        print("Error in collecting user input. Please try again.")
        return

    gender, age, temperature, systolic_bp, diastolic_bp, symptoms = user_data

    # Perform diagnosis
    diagnosis, severity = predict_diagnosis(
    ...
    engine.say(f"Your diagnosis is {diagnosis} with {severity} severity.")
    engine.runAndWait()

    # Provide suggestions based on diagnosis and severity
    suggestions = provide_suggestions(diagnosis, severity, gender, age, symptoms)
    print("\nSuggestions:")
    print(suggestions)
    engine.say("Here are your suggestions.")
    engine.say(suggestions)
    engine.runAndWait()

    # Final message
    print("\nThank you for using the AI Diagnostic Machine. Stay healthy!")
    engine.say("Thank you for using the AI Diagnostic Machine. Stay healthy!")
    engine.runAndWait()
